...
DEFAULT_SSID="FedoraHotspot"
DEFAULT_PASS="ChangeMe123!"
DEFAULT_CONN="fedora_hotspot"
...
setup_hotspot() {
    ...
    # --- Delete all old Wi-Fi hotspot connections ---
    log_message INFO "Removing old Wi-Fi hotspot connections..."
    nmcli -t -f NAME,TYPE connection show | grep '^.*:wifi$' | cut -d: -f1 | while read -r old; do
        nmcli connection delete "$old" || true
        log_message INFO "Deleted old Wi-Fi connection: $old"
    done

    # --- Interactive prompts ---
    read -rp "Enter Hotspot connection name [${DEFAULT_CONN}]: " HOTSPOT_CONN_NAME
    HOTSPOT_CONN_NAME=${HOTSPOT_CONN_NAME:-$DEFAULT_CONN}

    read -rp "Enter Hotspot SSID [${DEFAULT_SSID}]: " HOTSPOT_SSID
    HOTSPOT_SSID=${HOTSPOT_SSID:-$DEFAULT_SSID}

    read -rsp "Enter Hotspot password (min 8 chars) [${DEFAULT_PASS}]: " HOTSPOT_PASSWORD
    echo
    HOTSPOT_PASSWORD=${HOTSPOT_PASSWORD:-$DEFAULT_PASS}

    if [[ ${#HOTSPOT_PASSWORD} -lt 8 ]]; then
        log_message ERROR "Password must be at least 8 characters"
        exit 1
    fi
...
case "${1:-}" in
    ...
    reset) nmcli -t -f NAME,TYPE connection show | grep '^.*:wifi$' | cut -d: -f1 | xargs -r -n1 nmcli connection delete ;;
